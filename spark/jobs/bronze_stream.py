"""
Bronze layer - Spark Structured Streaming job.

Objective:
    Read e-commerce events from Kafka,
    parse the raw JSON payload,
    preserve Kafka metadata for traceability,
    then persist the structured events into the Bronze layer.

Architecture : Kafka > Spark Structured Streaming > JSON parsing > Bronze(/lake/bronze)

The Spark checkpoint is stored separately : /lake/checkpoints/bronze then Spark can memorize the progression in the flow.
"""

from pyspark.sql import DataFrame
from pyspark.sql.functions import col, from_json

from utils.spark_session import create_spark_session
from config.config import KAFKA_BROKER, KAFKA_TOPIC, BRONZE_PATH, BRONZE_CHECKPOINT_PATH
from schemas.ecommerce_schema import EVENT_SCHEMA


# Transformation

def parse_kafka_events(kafka_df: DataFrame) -> DataFrame:
    """
    Transform raw Kafka records into structured Bronze records.
    Parameters:
    - kafka_df : Raw DataFrame returned by Spark's Kafka source.
    Returns:
    - Structured DataFrame containing business data
        and Kafka metadata.
    """


    # Payload JSON extraction
    # Convert value column returned by kafka to String before using from_json().
    # Conserve kafka metadata for traceability and for debugging

    json_df = kafka_df.select(
        # Payload JSON from our producer
        col("value").cast("string").alias("json_value"),

        # Kafka metadata kept for traceability
        col("topic").alias("kafka_topic"),
        col("partition").alias("kafka_partition"),
        col("offset").alias("kafka_offset"),
        col("timestamp").alias("kafka_timestamp"),
    )


    # Parse JSON into Spark structure using the schema defined in spark/schemas/ecommerce_schema.py

    parsed_df = (
        json_df
        .select(
            from_json(
                col("json_value"),
                EVENT_SCHEMA
            ).alias("data"),
            # Preserve Kafka metadata
            col("kafka_topic"),
            col("kafka_partition"),
            col("kafka_offset"),
            col("kafka_timestamp"),
        )
        .select(
            # Expand the parsed business data
            "data.*",
            # Add Kafka metadata to the Bronze record
            "kafka_topic",
            "kafka_partition",
            "kafka_offset",
            "kafka_timestamp",
        )
    )


    return parsed_df


# Main streaming application

def main():
    """
    Start the Bronze streaming pipeline.

    The main function contains the infrastructure-specific part
    of the job:
        - Spark session creation,
        - Kafka streaming source,
        - Bronze Parquet sink,
        - checkpoint management.
    """


    # Spark Session: Use of our centralized function to create Spark Session.

    spark = create_spark_session("EcommerceKafkaStreaming")


    # Kafka flow reading

    kafka_df = (
        spark.readStream
        .format("kafka")
        .option("kafka.bootstrap.servers", KAFKA_BROKER)
        .option("subscribe", KAFKA_TOPIC)
        .option("startingOffsets", "latest") # Read from newest data
        .load()
    )


    # Transform raw Kafka records into structured Bronze records.

    bronze_df = parse_kafka_events(kafka_df)


    # Write into the Bronze layer
    # Format: Parquet, Destination : /lake/bronze

    query = (
        bronze_df
        .writeStream
        .outputMode("append")
        .format("parquet")
        .option("path", BRONZE_PATH)
        .option(
            "checkpointLocation",
            BRONZE_CHECKPOINT_PATH
        )
        .start()
    )


    # Keep the job active

    query.awaitTermination() # awaitTermination() blocks the main process so that the Spark Streaming application continues to run.


# Application entry point

if __name__ == "__main__":
    main()